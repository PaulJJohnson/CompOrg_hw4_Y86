...
# 1. My initial change was to add the iaddq instruction as we did
#	in the last homework. The documentation for that is essentially
#	the same as it is here so I won't re-paste that.
# 2. Second change was to edit how the pipeline dealt with the execution
#	of a mrmovq instruction following a rmmovq instruction since this
#	happens extensively through the ncopy solution.
#	
#	When passing a value to e_valA, I implemented the following change;
#	
#	M_icode == IMRMOVQ && E_icode == IRMMOVQ &&
#	M_dstM == E_srcA && M_dstM != E_srcB
#	
#	For the D_stall, D_bubble, E_bubble, and F_stall, I implemented the
#	following change;
#
#	D_icode == IRMMOVQ && E_dstM == d_srcA && E_dstM != d_srcB
#
#	A description of these changes are included in-line.
#
#/* $begin pipe-all-hcl */
####################################################################
...
# Old: word e_valA = E_valA;    # Pass valA through stage
# New: 
#	Using M_valM instead of E_valE if a rmmovq instruction follows 
#	a mrmovq instruction.
#	1. Checks if the memory stage instruction and the execute stage instruction
#		are mrmovq and rmmovq instructions, respectively.
#
#		M_icode == IMRMOVQ && E_icode == IRMMOVQ
#
#	2. Checks if the destination of the memory stage is the source of the
#		execute stage and checks if the destination of the memory stage
#		is not from the execute stage's B source.
#
#		M_dstM == E_srcA && M_dstM != E_srcB
#
word e_valA = [
...
bool F_stall =
	...
	# Added a condition for the rmmovq instruction
	!(D_icode == IRMMOVQ && E_dstM == d_srcA && E_dstM != d_srcB) ||
